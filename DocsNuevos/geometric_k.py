...
from src.constants.base import NET_LABEL, STR_ZERO, ACTUAL, EFECTO, TYPE_TAG
...
def _particionar_variables_en_k(n_vars: int, k: int):
    ...
    con la restricción de que cada grupo tenga al menos 1 elemento.

    Devuelve listas de asignaciones: cada asignación es un array de longitud
    ...
    # Números de Stirling de segundo tipo serían el conteo exacto, pero
    # generamos directamente con producto cartesiano y filtramos.
    # Para eficiencia, si n_vars * k > 20 usamos heurística greedy en su lugar.
    if n_vars * k <= 20:
        for asig in iterproduct(range(k), repeat=n_vars):
            if len(set(asig)) == k:          # todos los grupos presentes
                yield list(asig)
    else:
        # señal de que el llamador debe usar heurística
        return


def _asignacion_a_grupos(asig: List[int], k: int) -> List[List[int]]:
    """Convierte array de asignaciones en lista de k grupos de índices."""
...
def _costo_total_biparticion_grupos(
    ...
    """
    Suma los costos de transición de las variables asignadas a cada grupo
    respecto al estado inicial → final, usando la tabla T ya calculada.
    
    Un buen agrupamiento tendrá costos bajos DENTRO de cada grupo y altos
    ...
    for grupo in grupos_futuros:
        # Penalización intra-grupo: queremos que variables del mismo grupo
        # tengan costos similares (baja discrepancia entre sí indica
...
class KPartitionGeometricSIA(SIA):
    """
    Extensión geométrica de GeometricSIA para k-particiones.

    Idea central
    ------------
    La tabla de costos T (heredada del enfoque de bipartición) mide la
    ...
    usando esa tabla como guía, minimizando la varianza intra-grupo (variables
    ...
    Después de identificar candidatos geométricamente, evaluamos cada
    candidato con la EMD real (igual que en el caso k=2) y devolvemos
    la k-partición con menor pérdida.

    Parámetro k
    -----------
    Se puede pasar `k` al constructor. Por defecto k=2 (compatible con
    GeometricSIA). Para k>2, si el número de variables es pequeño se hace
    búsqueda exacta; si es grande se usa heurística greedy basada en T.
    """
    ...
    def aplicar_estrategia(
        ...
        # Aplanar ncubos para acceso O(1) por índice de estado
        self._flat_data = [
    ...
    # ------------------------------------------------------------------
    # Construcción de la tabla de costos (igual a GeometricSIA)
    # ------------------------------------------------------------------
    ...
    def _calcular_costo(self, estado_ini: list, estado_fin: list):
        """Calcula recursivamente t(i,j) con factor γ = 2^{-d_H}."""
    ...
    # ------------------------------------------------------------------
    # Identificación de candidatos para k-particiones
    # ------------------------------------------------------------------

    def _candidatos_geometricos(self, k: int) -> List[List[List[int]]]:
        """
        Genera candidatos de k-partición usando la tabla T.

        Cada candidato es una lista de k grupos, donde cada grupo contiene
        índices de variables futuras (y opcionalmente presentes).
        ...
        4. Completamos con presentes: todos los presentes van en todos los grupos
           (como en la bipartición original) o los distribuimos según la misma
           lógica de costos si hay suficientes presentes.

        Para sistemas pequeños (n_vars * k <= 20) probamos todas las asignaciones.
        """
        ...
        if n_vars * k <= 20:
            ...
            for asig in _particionar_variables_en_k(n_vars, k):
                ...
                # Presentes: todos al primer grupo (como en bipartición base)
                # Nota: esto puede extenderse; aquí preservamos compatibilidad
                grupos_pres = [list(range(n_presentes))] + [[] for _ in range(k - 1)]
                candidatos.append([grupos_pres[i] + grupos_fut[i] for i in range(k)])
            return candidatos

        # ---- Caso heurístico greedy (sistemas grandes) ----
    ...
    def _candidatos_greedy(
        ...
        Estrategia 2 – Complemento:
            ...
            genera una bipartición que las separa (igual que GeometricSIA base)
        ...
        # Estrategia 3: rotación de grupos
    ...
    def _canonicalizar(asig: List[int], k: int) -> tuple:
        """
        Normaliza una asignación de grupos para detectar permutaciones repetidas.
    ...
    # ------------------------------------------------------------------
    # Evaluación real de candidatos (EMD)
    # ------------------------------------------------------------------
    ...
    def _evaluar_candidato(
        ...
        """
        Para una k-partición dada como lista de grupos de índices de variables,
        calcula la EMD entre la distribución marginal reconstruida y el original.
        ...
        el producto tensorial, igual a como se define δ en el documento.

        Devuelve (emd, dist_particionada) o None si el candidato es inválido.
        """
        dims = self.sia_subsistema.dims_ncubos
        idxs = self.sia_subsistema.indices_ncubos

        # Separar índices presentes (ACTUAL) y futuros (EFECTO) por grupo
        # Convención: los índices < len(dims) son presentes, >= son futuros
        n_pres = len(dims)
        ...
        # Fallback: si no hay separación explícita de presentes, asignamos
        ...
        # Calcular distribución marginal para cada parte
        dists = []
        ...
        # Para k>2: producto tensorial de distribuciones (reduce a escalar EMD)
        if len(dists) == 1:
            dist_final = dists[0]
        else:
            dist_final = dists[0]
            ...
            # Normalizar para que sea distribución de probabilidad válida
            s = dist_final.sum()
            if s > 0:
    ...
    def _find_k_mip(self):
        """
        Encuentra la k-partición de Mínima Pérdida de Información (k-MIP).

        Flujo:
        1. Construir tabla de costos T (BFS modificado).
        2. Generar candidatos geométricos de k grupos.
        3. Evaluar cada candidato con EMD real.
        4. Retornar la clave del mínimo.
        """
        self.logger.critic(f"k-MIP geométrico iniciando (k={self.k}).")
