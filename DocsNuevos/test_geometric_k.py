...
Verifican las rúbricas del PDF2 §5.2.1 sin depender de la red completa.
...
# ── Funciones puras extraídas de geometric_k.py para tests unitarios ──────
...
def _calcular_costo_standalone(
    ...
    """
    Versión standalone de _calcular_costo para tests sin dependencias del proyecto.
...
def flat_data_n3c():
    ...
    """
    return [
        np.array([0., 1., 0., 1., 0., 1., 0., 1.]),  # A
        np.array([0., 0., 1., 1., 0., 0., 1., 1.]),  # B
        np.array([0., 0., 0., 0., 1., 1., 1., 1.]),  # C
    ]
...
class TestTablaCostos:
    ...
    def test_costo_000_111_valor_critico_pdf(self, flat_data_n3c):
        """
        CRÍTICO: t(000,111) = 0.21875 para A, B y C.
    ...
    def test_factor_decrecimiento_exponencial(self, flat_data_n3c):
        """El factor γ = 2^{-d_H} debe aplicarse correctamente."""
        # d_H(000, 100) = 1 → γ = 0.5
        r1 = _calcular_costo_standalone([0, 0, 0], [1, 0, 0], flat_data_n3c)
        # d_H(000, 110) = 2 → γ = 0.25
        r2 = _calcular_costo_standalone([0, 0, 0], [1, 1, 0], flat_data_n3c)
        # Para B: r1[1]=0.0, r2[1]=0.375 — la relación no es simple pero
        # verificamos que el nivel-2 incorpora contribuciones de los vecinos
        assert r2[1] > r1[1] or r2[1] == r1[1], \
            "El costo a distancia 2 debe incluir contribuciones de los vecinos"


# ── Tests de particionamiento ─────────────────────────────────────────────

class TestParticionamiento:
    """Verifica la lógica de generación de k-particiones."""
    ...
    def test_grupos_son_disjuntos_y_cubren_todo(self):
        """Para cualquier asignación, los grupos cubren todos los índices exactamente."""
    ...
    def test_canonicalizacion_detecta_equivalencias(self):
        """[0,1,0] y [1,0,1] son la misma partición (renombrado de grupos)."""
        c1 = _canonicalizar([0, 1, 0], 2)
        c2 = _canonicalizar([1, 0, 1], 2)
        assert c1 == c2, "Deben ser la misma partición canónica"
...
class TestMetricas:
    """Verifica las métricas del benchmark (PDF2 §5.2.2)."""

    def test_error_relativo_cero_cuando_iguales(self):
        from DocsNuevos.benchmark import error_relativo_phi
    ...
    def test_jaccard_identicos_es_cero(self):
        from DocsNuevos.benchmark import distancia_jaccard
    ...
    def test_jaccard_alineacion_cruzada(self):
        """La distancia debe ser la mínima sobre las dos alineaciones."""
        from DocsNuevos.benchmark import distancia_jaccard
        # Si A=(S1,S2) y B=(S2,S1), la distancia cruzada debe ser 0
    ...
    def test_clasificar_excelente(self):
        from DocsNuevos.benchmark import clasificar_calidad
    ...
    def test_speedup_mayor_uno_cuando_pyphi_mas_lento(self):
        from DocsNuevos.benchmark import speedup
