"""
benchmark_mejorado.py — Evaluación de GeometricSIA / KPartitionGeometricSIA
contra PyPhi (referencia exhaustiva), según métricas del Cap. 5.2.2.

Uso:
    python benchmark_mejorado.py --excel ../../results/Pruebas_Metodo2.xlsx --k 2
...
def main():
    ...
    if KPartitionGeometricSIA is None:
        ...
        print("   Verifica que los módulos estén disponibles.")
        return
    ...
    for idx, fila in df.iterrows():
        print(f"  Procesando caso {idx+1}/{len(df)}: {fila.get('caso', '?')} ...")
